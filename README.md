...
> **Want Codex to execute but Gemini to review locally?** See [`docs/CODEX_GEMINI_REVIEW_GUIDE.md`](docs/CODEX_GEMINI_REVIEW_GUIDE.md) and [CN](docs/CODEX_GEMINI_REVIEW_GUIDE_CN.md). This path uses lightweight repo-local runners instead of MCP review bridges. For first-time users, the shortest path is: make sure `codex` works locally, set `GEMINI_API_KEY`, then run the guide's single-review smoke test.
